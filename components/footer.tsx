...
const ClearCompleted = FooterData.map((item, index) => {
  ...
  return (
    ...
    <FooterItem darkMode={darkMode} onClick={() => { setFilter("Completed")}}>Completed</FooterItem> */}
            <MobileWrapper>
            <SelectorDisplay style={{justifyContent: 'space-between', padding: '1.5em'}} darkMode={darkMode}>
                <SelectorItem>{taskCount} {ItemPluralization} Left</SelectorItem>
                <SelectorItem onClick={() => {setTodoList(todoList.filter((task) => {return task.status === 'active'}))}}>Clear Completed</SelectorItem>
            </SelectorDisplay>
            </MobileWrapper>
        <SelectorDisplay style={{ padding: '1.5em'}} darkMode={darkMode}>
            <DesktopWrapper><SelectorItem>{taskCount} {ItemPluralization} Left</SelectorItem></DesktopWrapper>
            <div style={{display: 'flex', alignItems: 'center'}}>{ClearCompleted}</div>
            <DesktopWrapper><SelectorItem onClick={() => {setTodoList(todoList.filter((task) => {return task.status === 'active'}))}}>Clear Completed</SelectorItem></DesktopWrapper>
        </SelectorDisplay>
    </FooterContainer>
  )
}
